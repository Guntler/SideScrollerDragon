...
GameMap::GameMap()
{
	...
	for (size_t i = 0; i < 10; i++){
		for (size_t j = 0; j < 10; j++){
			layer1[i][j] = new bases::Tile(0, 0, 0, 0, 16, 16, 1, 1);
	...
	for (size_t i = 0; i < 10; i++){
		layer1[9][i] = new bases::Tile(0, 0, 16, 16, 16, 16, 1, 1);
		layer1[9][i]->setPosition(16 * i, 16 * 10);
...
void GameMap::drawBackground(RenderWindow * window, Texture tileset)
{
	for (size_t i = 0; i < 10; i++){
		for (size_t j = 0; j < 10; j++){
			sf::Sprite temp = sf::Sprite(tileset);
